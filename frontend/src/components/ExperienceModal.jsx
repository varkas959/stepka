    ...
    try {
      ...
      if (!ok) {
        ...
        toast.error(
          ...
          : hasInj ? 'Remove instruction-like text â€” just describe the questions you were asked.'
      ...
      toast.success(
        bits.length ? `Experience added â€” ${bits.join(', ')}.` : 'Interview experience added. Thank you for contributing.',
        { duration: 5000 }
      );
      ...
      <DialogContent data-testid="experience-modal" className="max-w-3xl bg-zinc-950 border border-white/10 text-zinc-50 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-zinc-400 mt-1">
            Help thousands of candidates. Your report builds real company intelligence â€” rounds, questions, difficulty, and outcomes.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={submit} className="space-y-5 mt-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-zinc-500 mb-1.5">difficulty (1â€“5)</div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {rounds.map((r, ri) => (
                <div key={ri} className="rounded-md border border-white/8 p-3" style={{ background: '#13161D' }}>
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <input value={r.topics} onChange={e => setRound(ri, 'topics', e.target.value)}
                    placeholder="topics asked (comma-separated) â€” e.g. Promises, React Performance, System Design"
                    ...
                    {r.questions.map((q, qi) => (
                      <div key={qi} className="flex items-start gap-2">
                        <textarea value={q} onChange={e => setRoundQuestion(ri, qi, e.target.value)} rows={1}
                          placeholder={`Question ${qi + 1} you were askedâ€¦`}
                  ...
                  <input value={r.notes} onChange={e => setRound(ri, 'notes', e.target.value)}
                    placeholder="round notes (optional) â€” vibe, format, what they focused on"
          ...
          <div>
            ...
            <textarea value={form.notes} onChange={e => set('notes', e.target.value)} rows={3}
              placeholder="Overall experience, prep advice, timeline, anything that would help the next candidateâ€¦"
          ...
          <div className="flex items-center gap-2 pt-1">
            <button type="submit" data-testid="exp-submit" disabled={submitting}
              ...
              {submitting ? 'Submittingâ€¦' : 'Submit experience'}
            </button>
